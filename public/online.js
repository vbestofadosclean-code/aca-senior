...
(function(){
  ...
  window.showNewDetail=function(i){const r=window.__regs?.[i];if(!r)return;const d=$('databaseDetail');const created=r.createdAt?new Date(r.createdAt).toLocaleString('pt-AO',{dateStyle:'short',timeStyle:'medium'}):'Não disponível';const fields=[['Número',r.registrationNumber],['Nome completo',r.nome],['Nascimento',r.nascimento],['Sexo',r.sexo],['Naturalidade',r.naturalidade],['Bilhete',r.bilhete],['Residência',r.residencia],['Contacto',r.contacto],['Email',r.email],['Agrupamento',r.agrupamento],['Núcleo',r.nucleo],['Patrulha',r.patrulha],['Categoria',r.categoria],['Deficiência',r.deficiencia],['Observações',r.obs||'—'],['Data e hora da inscrição',created],['Estado',r.paymentStatus==='paid'?'Pago':'Pendente']];d.innerHTML='<h2>Informações completas da inscrição</h2><div class="detail-grid">'+fields.map(x=>`<div><small>${escHtml(x[0])}</small><strong>${escHtml(x[1])}</strong></div>`).join('')+'</div>';d.style.display='block';d.scrollIntoView({behavior:'smooth'});};
